...
public class WSQueueManager implements Runnable {
    ...
    public void startSlice(WSOrder wsOrder) throws Throwable {
    ...
    public void run() {
        while (true) try {
            List<WSOrder> wsOrderList = checkOrders();

            HashSet<Thread> threadsStartSlice = new HashSet<>();
            for (WSOrder wsOrder : wsOrderList) {
                ...
                threadsStartSlice.add(thread);
                thread.start();
            }
            for (Thread t : threadsStartSlice) {
                t.join();
            }
            Thread.sleep(3000);
        } catch (Throwable throwable) {
